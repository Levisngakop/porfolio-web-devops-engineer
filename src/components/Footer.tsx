import React from 'react';
import { Github, Linkedin, Mail, Heart, ArrowUp } from 'lucide-react';

const Footer: React.FC = () => {
  const scrollToTop = () => window.scrollTo({ top: 0, behavior: 'smooth' });

  return (
    <footer className="bg-slate-900 border-t border-slate-700 py-12 px-6">
      <div className="container mx-auto">
        <div className="grid md:grid-cols-4 gap-8 mb-12">

          {/* Brand */}
          <div className="md:col-span-2">
            <h3 className="text-2xl font-bold mb-4 bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">
              Levis Ngakop
            </h3>
            <p className="text-slate-400 mb-4 max-w-md">
              Cloud Native DevOps Engineer passionate about building resilient infrastructure
              and empowering teams through automation and best practices.
            </p>
            <div className="flex space-x-4">
              <a
                href="https://github.com/levisdevops"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="GitHub"
                className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition-colors"
              >
                <Github size={20} />
              </a>
              <a
                href="https://linkedin.com/in/levisngakop"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="LinkedIn"
                className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition-colors"
              >
                <Linkedin size={20} />
              </a>
              <a
                href="mailto:levisngakop47@gmail.com"
                aria-label="Email"
                className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition-colors"
              >
                <Mail size={20} />
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="font-semibold mb-4 text-slate-300">Quick Links</h4>
            <ul className="space-y-2">
              {['about','projects','skills','blog','contact'].map((sec) => (
                <li key={sec}>
                  <button
                    onClick={() => document.getElementById(sec)?.scrollIntoView({ behavior: 'smooth' })}
                    className="text-slate-400 hover:text-blue-400 transition-colors focus:outline-none focus:ring-1 focus:ring-blue-400"
                  >
                    {sec.charAt(0).toUpperCase() + sec.slice(1)}
                  </button>
                </li>
              ))}
            </ul>
          </div>

          {/* Services */}
          <div>
            <h4 className="font-semibold mb-4 text-slate-300">Services</h4>
            <ul className="space-y-2 text-slate-400">
              <li>Cloud Architecture</li>
              <li>DevOps Consulting</li>
              <li>CI/CD Implementation</li>
              <li>Infrastructure Automation</li>
            </ul>
          </div>

        </div>

        {/* Bottom Bar */}
        <div className="border-t border-slate-700 pt-8 flex flex-col md:flex-row justify-between items-center">
          <div className="flex items-center text-slate-400 mb-4 md:mb-0">
            <span>Made with</span>
            <Heart size={16} className="mx-2 text-red-500" />
            <span>React, TypeScript, and Tailwind CSS</span>
          </div>

          <div className="flex items-center space-x-4">
            <p className="text-slate-400">© 2025 Levis Ngakop. All rights reserved.</p>
            <button
              onClick={scrollToTop}
              aria-label="Scroll to top"
              className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition-colors"
            >
              <ArrowUp size={16} />
            </button>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;

