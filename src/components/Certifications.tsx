      ...
      <div className="container mx-auto">
        ...
        <div className="mb-16">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {certifications.map((cert) => (
              <div
                key={cert.id}
                className="bg-slate-800 rounded-lg p-6 border border-slate-700 hover:border-blue-500 transition-all duration-300 transform hover:scale-105"
              >
                <div className="text-center mb-6">
                  ...
                  <div className="flex items-center justify-center mb-2">
                    {cert.status === 'completed' && (
                      <span className="px-3 py-1 bg-green-600 text-green-100 rounded-full text-sm mr-2">
                        Certified
                      </span>
                    )}
                    {cert.status === 'in-progress' && (
                      <span className="px-3 py-1 bg-blue-600 text-blue-100 rounded-full text-sm mr-2">
                        In Progress
                      </span>
                    )}
        ...
        <div className="bg-slate-800 rounded-lg p-8">
          <h3 className="text-2xl font-semibold mb-6 text-center">
            <Award className="inline-block mr-2 text-yellow-400" size={28} />
            Learning Roadmap
          </h3>
          <div className="grid md:grid-cols-2 gap-6">
            {upcomingCertifications.map((cert, index) => (
              <div
                key={index}
                className="bg-slate-700 rounded-lg p-6 border-l-4 border-blue-400"
              >
